...
CREATE TABLE _type_bac (type_bac VARCHAR(200),
CONSTRAINT type_bac_pk PRIMARY KEY(type_bac));

CREATE TABLE _mention_bac (libelle_mention VARCHAR(25),
...
CREATE TABLE _admissions_generalites (cod_aff_form VARCHAR(50), session_annees INTEGER, selectivite VARCHAR(25), capacite INTEGER,effectif_total_candidats INTEGER, effectif_total_candidates INTEGER,
...
CONSTRAINT admission_gen_fk_session FOREIGN KEY ( sesssion_annee) REFERENCES _session(session_annee));
...
CONSTRAINT filiere_formation_fk_filiere (filiere_id) REFERENCES _filiere(filiere_id),
CONSTRAINT filiere_formation_fk_formation (cod_aff_form) REFERENCES _formation(cod_aff_form));
...
CONSTRAINT academie_formation_fk_academie (academie_nom) REFERENCES _academie(academie_nom),
CONSTRAINT academie_formation_fk_formation (cod_aff_form) REFERENCES _formation(cod_aff_form));
...
CONSTRAINT etablissement_formation_fk_etablissement (etablissement_code_uai) REFERENCES _etablissement(etablissement_code_uai),
CONSTRAINT etablissement_formation_fk_formation (cod_aff_form) REFERENCES _formation(cod_aff_form));
...
CONSTRAINT commune_formation_fk_commune (commune_nom) REFERENCES _commune(commune_nom),
CONSTRAINT commune_formation_fk_formation (cod_aff_form) REFERENCES _formation(cod_aff_form));

CREATE TABLE _commune_departement (commune_nom VARCHAR(35), departement_code VARCHAR(3), 
CONSTRAINT commune_departement_pk PRIMARY KEY (commune_nom, cod_aff_form),
CONSTRAINT commune_departement_fk_etablissement (commune_nom) REFERENCES _commune(commune_nom),
CONSTRAINT commune_departement_fk_departement (departement_code) REFERENCES _departement(departement_code));

CREATE TABLE _region_departement (region_nom VARCHAR(35), departement_code VARCHAR(3), 
CONSTRAINT region_departement_pk PRIMARY KEY (region_nom, cod_aff_form),
CONSTRAINT region_departement_fk_region (region_nom) REFERENCES _commune(region_nom),
CONSTRAINT region_departement_fk_departement (departement_code) REFERENCES _departement(departement_code));
